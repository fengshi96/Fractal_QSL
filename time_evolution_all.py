...
def build_triangle_product_state(lattice):
    alpha_1 = (-np.sqrt(3) + 3j) / 6.0
    alpha_2 = (-(np.sqrt(3) + 3j)) / 6.0
    alpha_3 = 1.0 / np.sqrt(3)

    psi = np.zeros(lattice.n_vertices, dtype=np.complex128)
    triangles = [p for p in lattice.plaquettes if len(p.vertices) == 3]
    if len(triangles) == 0:
        raise ValueError("No triangular plaquettes found to build g_i^\u2020 product state.")
...
def create_time_evolution_animation(lattice, wavefunctions, total_time, output_gif, target_flux=None, cmap="Purples", fps=5, vmin=0, vmax=0.01):
    ...
    def update(frame):
        ...
        if frame % progress_every == 0 or frame == total_frames - 1:
            print(f"Rendering frame {frame + 1}/{total_frames}, t={np.round(t, 2)}")
...
def main(total, cmdargs):
    ...
    psi_0 = build_triangle_product_state(modified_lattice)
    ...
    psi_t = time_evolution(maj_energies, maj_states, psi_0, times)
    ...
    create_time_evolution_animation(
        ...
        cmap="ocean_r",
        target_flux=None,
        output_gif="time_evolution_all.gif",
        fps=fps,
        vmin=0.001,
        vmax=0.005
    )
