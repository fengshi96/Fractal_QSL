...
def main(total, cmdargs):
    ...
    highest_points = 9000
    level = 0
    Ns = np.zeros((highest_points - lowest_points + 500)//500, dtype=int)  # the number of sites at each level
    for s, seed in enumerate(Seeds):
        Gaps = np.zeros((highest_points - lowest_points + 500)//500)
        for i, points in enumerate(range(lowest_points, highest_points + 500, 500)):
            modified_lattice, coloring_solution = amorphous_Sierpinski(Seed=seed, init_points=points, fractal_level=level, open_bc=False) #424
    ...
    for s, seed in enumerate(Seeds):
        Gaps = all_Gaps[s]
        ax.plot(1/Ns, Gaps, marker=next(markers_cycle), color=next(colors_cycle), ms = 8, fillstyle='none', linestyle='')
    ...
    ax.set_xlabel(r"$1/N$", fontsize=18)
