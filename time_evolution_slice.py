...
def plot_time_slice(lattice, wavefunctions, output_file, target_flux=None, cmap="Purples", vmin=0, vmax=0.01, time_index=None, time_value=None):
    ...
    if wavefunctions.ndim == 1:
        wf = wavefunctions
        actual_time = 0 if time_value is None else time_value
    else:
        n_times = wavefunctions.shape[0]
        if time_value is not None:
            time_index = int(np.argmin(np.abs(np.arange(n_times) - time_value)))
            actual_time = time_value
        else:
            if time_index is None:
                time_index = 0
            time_index = max(0, min(time_index, n_times - 1))
            actual_time = time_index

        wf = wavefunctions[time_index]
    ...
    ax.axis("equal")
    ax.axis("off")
    ax.text(0.02, 0.98, f"t={np.round(actual_time, 2)}", transform=ax.transAxes, fontsize=12, verticalalignment="top")

    plt.tight_layout(pad=0)
    plt.savefig(output_file, dpi=300, bbox_inches='tight')
    print(f"Time-slice saved to {output_file}")
...
def main(total, cmdargs):
    ...
    plot_time_slice(
        modified_lattice,
        psi_t[:, 0],
        cmap="ocean_r",  # gist_heat_r, ocean_r
        target_flux=None,
        output_file=f"time_slice_{slice_time}.pdf",
        vmin=0.002,
        vmax=0.01,
