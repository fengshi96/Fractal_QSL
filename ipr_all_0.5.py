...
def main(total, cmdargs):
    ...
    for l in List:
        ...
        # Filter the energies EXCLUDING Low ENERGY & IPR into the desired window
        energy_range_min = 0.1
        energy_range_max = 1.5
        filtered_indices = np.where(
            (maj_energies >= energy_range_min) &
            (maj_energies <= energy_range_max)
        )[0]
        filtered_ipr_b = ipr_values[filtered_indices]
        filtered_ipr_list_b.append(filtered_ipr_b)

        # Filter the energies ONLY for GS ENERGY & IPR into the desired window
        filtered_ipr_c = data['ipr'][0, len(maj_energies) // 2]
   ...
   # colorbar for IPR
    ...
    avg_ipr_L = np.array([np.mean(arr) for arr in filtered_ipr_list_b])
